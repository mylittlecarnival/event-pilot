...
import { getInvoiceDisclosuresByApprovalHash, approveInvoiceDisclosure } from '@/lib/api/disclosures'
import type { InvoiceApprovalDisclosure } from '@/types/disclosures'

interface InvoiceApprovalDisclosuresProps {
  approvalHash: string
  onDisclosureStatusChange: (allApproved: boolean) => void
}

export function InvoiceApprovalDisclosures({ approvalHash, onDisclosureStatusChange }: InvoiceApprovalDisclosuresProps) {
  const [disclosures, setDisclosures] = useState<InvoiceApprovalDisclosure[]>([])
  ...
  useEffect(() => {
    ...
    async function loadDisclosures() {
      ...
      try {
        const data = await getInvoiceDisclosuresByApprovalHash(approvalHash)
        if (isMounted) {
          setDisclosures(data)
          setHasLoaded(true)
        }
        ...
        if (isMounted) {
          console.error('Error loading invoice disclosures:', error)
  ...
  // Check if all disclosures are approved and notify parent
  useEffect(() => {
    const allApproved = disclosures.length === 0 || disclosures.every(disclosure => disclosure.is_approved)
    onDisclosureStatusChange(allApproved)
  }, [disclosures, onDisclosureStatusChange])

  const handleDisclosureToggle = async (disclosure: InvoiceApprovalDisclosure) => {
    ...
    try {
      const updatedDisclosure = await approveInvoiceDisclosure(disclosure.id, true)
      setDisclosures(prev => prev.map(d =>
        d.id === disclosure.id ? updatedDisclosure : d
      ))
    } catch (error) {
      console.error('Error updating disclosure:', error)
      setError('Failed to update disclosure approval')
    }
  }
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-600">
          Please review and approve the following disclosures before approving this invoice:
        </p>
      </div>
      ...
      {disclosures.some(d => !d.is_approved) && (
        <div className="rounded-md bg-amber-50 p-4">
          <div className="text-sm text-amber-800">
            <strong>Note:</strong> You must approve all disclosures above before you can approve this invoice.
          </div>
        </div>
      )}
    </div>
