...
export function NewInvoiceModal({ isOpen, onClose, onSuccess }: NewInvoiceModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (newInvoice) {
        // Add default products to the new invoice
        try {
          const defaultProducts = await getDefaultProducts()
          console.log('Adding default products to new invoice:', defaultProducts.length)
          ...
          if (defaultProducts.length > 0) {
            ...
            console.log('Successfully added default products to new invoice')
          }
        } catch (error) {
  ...
  return (
    <Dialog open={isOpen} onClose={handleClose}>
      ...
      <DialogDescription>
        Create a new invoice by selecting a contact and providing basic event details.
      </DialogDescription>
