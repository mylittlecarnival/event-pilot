...
export function ApprovalRequestModal({ isOpen, onClose, estimate }: ApprovalRequestModalProps) {
  ...
  const handleSubmit = async () => {
    if (!estimate.contact_id) {
      setError('No contact is assigned to this estimate. Please assign a contact before sending approval request.')
      return
    }

    if (!dueDate) {
      setError('Please select a due date for the approval request.')
      return
    }
    ...
    try {
      // First, add selected disclosures to estimate if any are selected
      const selectedDisclosureIds = disclosures.filter(d => d.selected).map(d => d.id)
      if (selectedDisclosureIds.length > 0) {
        await addDisclosuresToEstimate(estimate.id, estimate.contact_id, selectedDisclosureIds)
      }

      // Then send the approval request
      const response = await fetch(`/api/estimates/${estimate.id}/send-approval`, {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Failed to send approval request')
      }
      ...
      <Dialog open={isOpen} onClose={handleClose}>
        ...
        <DialogActions>
          ...
          <Button
            ...
            disabled={isSubmitting || !estimate.contact_id || !dueDate || loadingDisclosures}
          >
